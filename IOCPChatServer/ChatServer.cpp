...
bool ChatServer::MoniterThread_serv()
{
	...
	while (true)
	{
		...
		{			
			...
			wprintf(L"[Update               ] Enq Cnt  : %10I64d   Thread Cnt : %10I64d\n", InterlockedExchange64(&m_jobUpdatecnt, 0), InterlockedExchange64(&m_jobThreadUpdateCnt, 0));
			wprintf(L"[Job Pool             ] Capacity : %10llu   Use        : %10llu    Alloc : %10llu    Free : %10llu\n",
				jobPool.GetCapacity(), jobPool.GetObjectUseCount(), jobPool.GetObjectAllocCount(), jobPool.GetObjectFreeCount());
