...
class TLSObjectPool
{
	...
	public:
		...
		Bucket(int iNodeNum, bool _placementNew, LockFreeStack<Node*>* rootPool) :
			maxCount(NODEMAX), size(iNodeNum), allocCount(0), freeCount(0), pTop(nullptr),
...
private:
	...
	alignas(64) __int64 bucketAllocCount;				// 현재까지 버킷 할당한 개수 (TLS index당 버킷 2개지만 counting은 1개로 봄)
	...
	long defaultNodeCount;								// 처음 지정한 노드 개수
	bool m_bPlacementNew;								// placement new 작동 여부
	DWORD tlsIndex;										// tls Index
	LockFreeStack<Node*>* rootPool;						// root Pool
	//ObjectLockFreeStackFreeList<Bucket>* bucketPool;	// bucket Pool

public:
	TLSObjectPool(int iNodeNum = NODEMAX, bool bPlacementNew = false)
		: m_bPlacementNew(bPlacementNew), m_iCapacity(0), bucketAllocCount(0),
		objectAllocCount(0), objectFreeCount(0), defaultNodeCount(iNodeNum)
	{
		// 버킷 개수만큼 스택프리리스트 노드 생성
		rootPool = new LockFreeStack<Node*>(BUCKETMAX, bPlacementNew);

		Init();

		//bucketPool = new ObjectLockFreeStackFreeList<Bucket>(BUCKETMAX);
	...
	__int64 GetBucketAllocCount() { return bucketAllocCount; }
	...
	DATA* Alloc()
	{
		...
		if (buckets == nullptr)
		{
			...
			//InterlockedIncrement64(&bucketAllocCount);
#endif
		}
