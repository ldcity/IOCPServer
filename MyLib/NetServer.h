...
public:
	...
	// Accept TPS
	inline __int64 GetAcceptTPS()
	{
		return acceptTPS;
	}

	// Accept TPS
	__int64 GetRecvMessageTPS()
	{
		return recvMsgTPS;
	}

	// Accept TPS
	__int64 GetSendMessageTPS()
	{
		return sendMsgTPS;
	}
...
protected:
	...
	// [RETURN] X 
	// ==========================================================
	virtual void OnError(int errorCode, wchar_t* msg) = 0;
...
private:
	...
	friend unsigned __stdcall ControlThread(void* param);
	friend unsigned __stdcall RunningThread(void* param);

	bool AcceptThread_serv();
	bool mWorkerThread_serv();
	bool mMoniterThread_serv();
	bool mControlThread_serv();
	bool mRunningThread_serv();
...
private:
	...
	HANDLE mUpdateThread;								// Job Queue Process Thread
	HANDLE mMoniteringThread;							// Monitering Thread
	HANDLE mControlThread;								// Control Thread
	HANDLE mRunningThread;
	vector<HANDLE> mTotalThreads;							// Total Thread Handle

	HANDLE RunEvent;									// Thread Start Event
	HANDLE MoniterEvent;								// Monitering Event
	...
	unsigned long long s_sessionUniqueID;				// Unique Value

	stSESSION* SessionArray;				// Session Array			
	//std::stack<unsigned int> AvailableIndexStack;		// Available Session Array Index
	...
	//CRITICAL_SECTION cs;
	//queue<__int64> sendDataQ;
	//queue<__int64> recvDataQ;

protected:
	// 모니터링용 변수 (1초 기준)
	// 이해의 편의를 위해 TPS가 들어간 변수는 1초당 발생하는 건수를 계산, 나머지는 총 누적 합계를 나타냄
	SYSTEMTIME curTime;									// Current Date
...
protected:
	struct Job
	{
		...
		__int64 type;				// job type
		__int64 data;				// job data
	};
	...
	HANDLE JobEvent;									// Job Enqueue Event

	ObjectLockFreeStackFreeList<Job> JobPool;

	LockFreeQueue<Job*> JobQ;
};
