...
bool NetServer::NetAcceptThread_serv()
{
	...
	while (true)
	{
		...
		if (clientSocket == INVALID_SOCKET)
		{
			...
			logger->logger(dfLOG_LEVEL_ERROR, __LINE__, L"accept error %d", acceptError);
		...
		if (CreateIoCompletionPort((HANDLE)clientSocket, IOCPHandle, (ULONG_PTR)session, 0) == NULL)
		{
			...
			logger->logger(dfLOG_LEVEL_ERROR, __LINE__, L"accept # iocp error %d", ciocpError);
...
bool NetServer::RecvProc(stSESSION* pSession, long cbTransferred)
{
	...
	while (useSize > 0)
	{
		...
		{
			logger->logger(dfLOG_LEVEL_ERROR, __LINE__, L"RecvProc # code error > code : %d", header.code);
		...
		{
			logger->logger(dfLOG_LEVEL_ERROR, __LINE__, L"RecvProc # len error > size : %d", header.len);
			DisconnectSession(pSession->sessionID);
		...
		// payload 크기만큼 데이터 Dequeue (헤더포함)
		pSession->recvRingBuffer.Dequeue(packet->GetNetBufferPtr(), header.len + CPacket::en_PACKET::WAN_HEADER_SIZE);
		...
		if (!packet->Decoding())
		{
			logger->logger(dfLOG_LEVEL_ERROR, __LINE__, L"RecvProc # decoding error");
